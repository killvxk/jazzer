...
public final class RegexRoadblocks {
  ...
  private static boolean DURING_JAZZER_STARTUP = true;

  static {
    Jazzer.onFuzzTargetReady(() -> DURING_JAZZER_STARTUP = false);
  }
  ...
  nodeMatchHook(MethodHandle method, Object node, Object[] args, int hookId) {
    // Do not act on instrumented regexes used by Jazzer internally.
    if (DURING_JAZZER_STARTUP)
      return;
    if (node == null)
      return;
